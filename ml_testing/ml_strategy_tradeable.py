"""
IBIT Overnight Strategy - Tradeable ML Model
=============================================
Uses ONLY features available at 3:50 PM (10 min before close)
to predict tonight's overnight return.

This model can actually be used in live trading, unlike the baseline
which uses end-of-day data not available before market close.

Features available at 3:50 PM:
- Calendar features (day of week, month, etc.)
- Previous overnight returns (yesterday and before)
- Prior day returns and volatility
- NO same-day close prices or volumes
"""
...
# Output directory
OUTPUT_DIR = os.path.join(os.path.dirname(__file__), 'output_tradeable')
...
def create_features(ibit_df, spy_df):
    """
    Create feature matrix using ONLY data available at 3:50 PM.
    
    Available: Yesterday's and prior data
    NOT Available: Today's close, today's full volume
    """
    print("🔧 Engineering TRADEABLE features (available at 3:50 PM)...")
    ...
    # This is what we're trying to predict BEFORE it happens
    ...
    # ============ PREVIOUS OVERNIGHT RETURNS (Available - from prior days) ============
    ...
    # Overnight streak (consecutive positive/negative nights)
    df['overnight_streak'] = df['prev_overnight_1'].apply(lambda x: 1 if x > 0 else -1)
    df['overnight_streak'] = df['overnight_streak'].groupby(
        (df['overnight_streak'] != df['overnight_streak'].shift()).cumsum()
    ).cumsum()
    
    # ============ PRIOR DAY RETURNS (Available - using yesterday's close) ============
    # These use YESTERDAY's close, not today's
    df['prior_day_return'] = df['ibit_close'].shift(1).pct_change(1)  # Day before yesterday to yesterday
    ...
    # Relative strength (using prior day data)
    ...
    # ============ PRIOR VOLATILITY (Available - based on prior closes) ============
    ...
    # ============ PRIOR VOLUME PATTERNS (Available - yesterday's volume) ============
    df['prior_volume'] = df['ibit_volume'].shift(1)
    df['prior_volume_ma5'] = df['ibit_volume'].shift(1).rolling(5).mean()
    df['prior_volume_ma20'] = df['ibit_volume'].shift(1).rolling(20).mean()
    df['prior_volume_ratio_5'] = df['prior_volume'] / df['prior_volume_ma5']
    df['prior_volume_ratio_20'] = df['prior_volume'] / df['prior_volume_ma20']
    
    df['spy_prior_volume'] = df['spy_volume'].shift(1)
    df['prior_ibit_spy_vol_ratio'] = df['prior_volume'] / df['spy_prior_volume']
    
    # ============ PRICE LEVEL FEATURES (Available - yesterday's close) ============
    ...
    print(f"   Created tradeable features")
...
def get_feature_columns():
    """Return list of TRADEABLE feature columns (available at 3:50 PM)."""
    return [
        # Datetime features (always available)
        ...
        # Prior volume patterns
        'prior_volume_ratio_5', 'prior_volume_ratio_20', 'prior_ibit_spy_vol_ratio',
...
def train_models(df):
    ...
    print("\n🤖 Training ML models (tradeable features only)...")
...
def plot_cumulative_performance(backtest_results, test_df):
    ...
    ax.set_title('TRADEABLE Strategy Comparison\n(Using only features available at 3:50 PM)', 
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'tradeable_cumulative_performance.png'), dpi=150, bbox_inches='tight')
    plt.close()
    print(f"   Saved: tradeable_cumulative_performance.png")
...
def plot_model_comparison(results):
    ...
    ax.set_title('TRADEABLE Model Performance Comparison', fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'tradeable_model_comparison.png'), dpi=150, bbox_inches='tight')
    plt.close()
    print(f"   Saved: tradeable_model_comparison.png")
...
def plot_shap_analysis(trained_models, X_train, X_test, feature_cols):
    ...
    for name, model in trained_models.items():
        try:
            ...
            plt.title(f'SHAP Summary (Tradeable): {name}', fontsize=14, fontweight='bold', y=1.02)
            plt.tight_layout()
            
            filename = f'tradeable_shap_{name.lower().replace(" ", "_")}.png'
...
def generate_pdf_report(ml_results, backtest_results, df, shap_plots=None):
    ...
    pdf_path = os.path.join(OUTPUT_DIR, 'tradeable_ml_report.pdf')
    ...
    # Title
    story.append(Paragraph("IBIT Overnight Strategy - TRADEABLE ML Model", title_style))
    story.append(Paragraph(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}", styles['Normal']))
    story.append(Spacer(1, 10))
    story.append(Paragraph(
        "<b>Key Difference:</b> This model uses ONLY features available at 3:50 PM, "
        "making it actually usable for live trading decisions.",
        styles['Normal']
    ))
    ...
    img_path = os.path.join(OUTPUT_DIR, 'tradeable_cumulative_performance.png')
    if os.path.exists(img_path):
        story.append(Image(img_path, width=7*inch, height=4*inch))
        story.append(Spacer(1, 20))
    
    img_path = os.path.join(OUTPUT_DIR, 'tradeable_model_comparison.png')
    if os.path.exists(img_path):
    ...
    if shap_plots:
        ...
        story.append(Paragraph(
            "SHAP values show how each tradeable feature contributes to predictions. "
            "All features shown are available at 3:50 PM for live trading.",
            styles['Normal']
        ))
...
def print_summary(ml_results, backtest_results):
    ...
    print("\n" + "="*80)
    print("TRADEABLE ML STRATEGY RESULTS (Features available at 3:50 PM)")
    print("="*80)
...
def compare_with_baseline():
    """Print comparison note with baseline model."""
    print("\n" + "="*80)
    print("📊 COMPARISON WITH BASELINE MODEL")
    print("="*80)
    print("""
    BASELINE MODEL (ml_strategy_analysis.py):
    - Uses end-of-day data (today's close, full volume)
    - NOT usable for live trading (data not available at decision time)
    - Random Forest: 25.8% return, 1.84 Sharpe
    
    TRADEABLE MODEL (this script):
    - Uses ONLY prior-day data
    - CAN be used for live trading at 3:50 PM
    - See results above for comparison
    
    Expected: Tradeable model will have lower performance since it has
    less information available. The question is whether it still beats
    "Always Night" strategy.
    """)
    print("="*80)


def main():
    """Run tradeable ML analysis."""
    print("="*60)
    print("IBIT Overnight Strategy - TRADEABLE ML Analysis")
    print("(Using only features available at 3:50 PM)")
    print("="*60)
    ...
    results_df.to_csv(os.path.join(OUTPUT_DIR, 'tradeable_backtest_results.csv'), index=False)
