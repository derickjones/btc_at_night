#!/usr/bin/env python3
"""
GitHub Actions adapter for ML-enhanced IBIT trading strategy.
...
# Import the ML strategy
from ml_strategy import MLOvernightStrategy
...
def should_buy_now():
    """Check if we should execute buy signal now (3:50 PM EST)."""
    now = get_est_now()
...
def should_sell_now():
    """Check if we should execute sell signal now (9:31 AM EST)."""
    now = get_est_now()
    if now.weekday() < 5:  # Monday = 0, Friday = 4
        current_time = now.time()
...
def main():
    """Main GitHub Actions ML trading execution."""
    setup_logging()
    ...
    print("="*60)
    print("🤖 GitHub Actions ML IBIT Strategy")
    print("="*60)
    ...
    try:
        ...
        if should_buy_now():
            print("\n🛒 Executing ML BUY signal...")
            print("   (Model will decide whether to actually buy)")
            strategy.execute_buy_signal()
        elif should_sell_now():
            print("\n🔄 Executing SELL signal...")
            strategy.execute_sell_signal()
        else:
            print("\n⏰ No trading action scheduled for current time")
        ...
        print("\n✅ ML Trading cycle completed successfully")
        
    except Exception as e:
        print(f"\n❌ Error in ML trading cycle: {e}")
